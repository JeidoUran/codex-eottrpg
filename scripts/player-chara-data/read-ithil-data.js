const ithilUrl =
  "https://s3.codex.eottrpg.memiroa.com/data/characters/ithil.json";
...
fetch(url)
  ...
  .then((json) => {
    ...
    const renderAbility = (item) => {
      const baseClassPage = "vagabond";
      const baseRacePage = "celestrien";
      const match = item.img.match(/data\/assets\/(.+)/);
      const relativePath = match ? match[1] : "placeholder.png";
      const iconPath = `../../assets/images/foundry-icons/${relativePath}`;
    ...
    document.getElementById("xp-bar-ithil").style.width =
