const oktarUrl =
  "https://s3.codex.eottrpg.memiroa.com/data/characters/oktar.json";
...
fetch(url)
  ...
  .then((json) => {
    ...
    const renderAbility = (item) => {
      const baseClassPage = "imperial";
      const baseRacePage = "sentinelle";
      const match = item.img.match(/data\/assets\/(.+)/);
      const relativePath = match ? match[1] : "placeholder.png";
      const iconPath = `../../assets/images/foundry-icons/${relativePath}`;
    ...
    const featuresDiv = document.getElementById("oktar-features");
    const features =
      data.items?.filter(
        (item) =>
    ...
    document.getElementById("xp-bar-oktar").style.width =
