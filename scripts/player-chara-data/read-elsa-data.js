const elsaUrl =
  "https://s3.codex.eottrpg.memiroa.com/data/characters/elsa.json";
...
fetch(url)
  ...
  .then((json) => {
    ...
    const stats = document.getElementById("elsa-stats");
    const inventoryDiv = document.getElementById("elsa-inventory");
    ...
    const renderAbility = (item) => {
      const baseClassPage = "lansquenet";
      const baseRacePage = "therian";
      const match = item.img.match(/data\/assets\/(.+)/);
      const relativePath = match ? match[1] : "placeholder.png";
      const iconPath = `../../assets/images/foundry-icons/${relativePath}`;
    ...
    const featuresDiv = document.getElementById("elsa-features");
    const features =
      data.items?.filter(
        (item) =>
    ...
    document.getElementById("xp-bar-elsa").style.width =
  ...
  .catch((err) => {
    console.error("Erreur lors du chargement des données Elsa :", err);
    document.getElementById("elsa-stats").innerHTML =
